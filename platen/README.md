...
* **The Hero (Platen):** 100% Permissive Open Core + Commercial Pro Template Vault. Runs as an ultra-fast HTTP sidecar responding to any language in $< 8\text{ms}$ with pure Typst templates.

---

## Quick Comparison

| Vector | Headless Chrome / Puppeteer | Oicana | **Platen Engine** |
| :--- | :--- | :--- | :--- |
...
```bash
python platen/core/server.py --port 8080
```
...
### 3. CLI Mode
```bash
python platen.py compile \
  --template platen/templates/b2b_invoice.typ \
  --data platen/data/b2b_invoice.json \
  --output invoice.pdf \
  --standard a-2b
```
...
Platen comes with production-tested, multi-page templates:
...
Run Platen as a hardened sidecar next to your application:

```bash
docker build -f platen/docker/Dockerfile -t platen:latest .
docker run -d -p 8080:8080 --name platen-sidecar platen:latest
```
...
* **Platen Pro ($490 / year):** Complete 25+ Template Vault, multi-worker HTTP sidecar, visual regression CLI, priority SLA.
...
European B2B transactions legally require hybrid electronic invoices compliant with **EN 16931** (Factur-X in France, ZUGFeRD in Germany). Platen packages human-readable PDF/A-3b containers with automatically generated, schema-validated UN/CEFACT CII XML (`factur-x.xml`) in $< 20\text{ms}$.

### 1. Compile Factur-X Invoice via CLI
```bash
python platen.py facturx \
  --template platen/templates/facturx_invoice.typ \
  --data platen/data/facturx_invoice.json \
...
curl -X POST http://localhost:8080/v1/facturx/render \
  -H "Content-Type: application/json" \
  -d @platen/data/facturx_invoice.json \
  --output invoice_facturx.pdf
```
...
```bash
python platen.py validate-facturx invoice_facturx.pdf
```
...
```bash
python platen.py extract-facturx invoice_facturx.pdf -o factur-x.xml
```

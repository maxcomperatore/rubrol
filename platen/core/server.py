"""
Platen Production Server & Sidecar
...
from platen.facturx.packager import extract_facturx_xml
...
class PlatenServerHandler(BaseHTTPRequestHandler):
    engine = PlatenEngine()
...
def serve(host: str = "0.0.0.0", port: int = 8080):
    server = ThreadedHTTPServer((host, port), PlatenServerHandler)
    print(f"[Platen Engine] Serving at http://{host}:{port} (PID: {os.getpid()})")
    print(f"[Platen Engine] Playground: http://localhost:{port}/")
    print(f"[Platen Engine] API Endpoint: POST http://localhost:{port}/v1/render")
    print(f"[Platen Engine] Factur-X Endpoint: POST http://localhost:{port}/v1/facturx/render")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\n[Platen Engine] Shutting down server...")
        server.shutdown()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Platen PDF Engine Server")

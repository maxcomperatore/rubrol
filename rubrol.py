...
DEFAULT_FACTURX_TEMPLATE = Path(__file__).parent / "rubrol" / "templates" / "facturx_invoice.typ"
DEFAULT_FACTURX_DATA = Path(__file__).parent / "rubrol" / "data" / "facturx_invoice.json"
...
class RubrolRequestHandler(BaseHTTPRequestHandler):
    engine = RubrolEngine()
...
def run_server(host: str = "0.0.0.0", port: int = 8080):
    server = ThreadedHTTPServer((host, port), RubrolRequestHandler)
    print(f"[Rubrol] Daemon listening on http://{host}:{port} (PID: {os.getpid()})")
    print(f"[Rubrol] Factur-X / ZUGFeRD Suite ready on /v1/facturx/render")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        print("\n[Rubrol] Stopping daemon...")
        server.shutdown()
